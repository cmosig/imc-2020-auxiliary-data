...
# ------------------------------------------------------------
assert (num_cores > 0)
assert (max_mis_rec_no_rfd >= 0 and max_mis_rec_no_rfd <= 1)
assert (min_alternative_occ >= 0 and min_alternative_occ <= 1)
assert (weight_metric_1 + weight_metric_2 + weight_metric_3 > 0.999
        and weight_metric_1 + weight_metric_2 + weight_metric_3 < 1.001)
assert (_get_entity_count([[1, 2, 3, 4], [1, 2]], _get_nodes) == {
    1: 2,
    2: 2,
    3: 1,
    4: 1
})
assert (_get_entity_count([[1, 2, 3, 4], [1, 2]], _get_links) == {
    (1, 2): 2,
    (2, 3): 1,
    (3, 4): 1
})
assert (_get_links([1, 2, 62]) == [(1, 2), (2, 62)])
assert (_get_links([]) == [])
assert (_get_links([1]) == [])
assert (_get_nodes([]) == [])
assert (_get_nodes([1, 2, 3]) == [1, 2, 3])
assert (_M1_rfd_path_ratio(_get_nodes, [[1, 2, 3], [3, 4, 5], [3, 12]],
                           [[1, 2, 4], [10, 20, 3, 40]]) == {
                               1: 0.5,
                               2: 0.5,
                               3: 0.75,
...
assert (_M1_rfd_path_ratio(_get_nodes, [[1, 2]], [[3, 4]]) == {
    1: 1.0,
    2: 1.0,
    3: 0.0,
    4: 0.0
})
assert (_M1_rfd_path_ratio(_get_links, [[1, 2]], [[3, 4]]) == {
    (1, 2): 1.0,
    (3, 4): 0.0
})
assert (_M1_rfd_path_ratio(
    _get_links, [[1, 2, 3], [3, 4, 5], [1, 2, 3, 12], [1, 2], [2, 3]],
    [[1, 2, 4], [10, 20, 2, 3, 40]]) == {
        (3, 40): 0.0,
        (2, 4): 0.0,
        (1, 2): 0.75,
