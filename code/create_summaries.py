...
datasets = [
    "rfd_paths_caitlin_format_1", "rfd_paths_caitlin_format_2",
    "rfd_paths_caitlin_format_3", "rfd_paths_caitlin_format_5",
    "rfd_paths_caitlin_format_10", "rfd_paths_caitlin_format_15"
]
dataset = datasets[0]
...
mcmc_samples = pd.read_csv(filepath + dataset + 'RFD_mcmc_samples.csv',
                           index_col=0)
print('found mcmc samples ')

hmc_samples = pd.read_csv(filepath + dataset + 'RFD_hmc_samples.csv',
                          index_col=0)
...
for w, each_path in enumerate(df[df.rfd].path.values):
    if max(results.loc[p].total_risk for p in each_path) <= 3:
        ...
        c_sort = sorted(Counter(
            hmc_samples.loc[each_path].apply(lambda x: np.argmin(x))).items(),
                        key=lambda kv: kv[1],
                        reverse=True)
